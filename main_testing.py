...
def getEAR(img,subjects):
    ...
    for subject in subjects:
        ...
        box = np.int0(box)
        cv2.drawContours(frame,[box],0,(150,100,100),2)

        mouthHullRotated = getSubImage(rect,mouthHull)
    ...
    view(frame)
    # cv2.imwrite("temp_"+str(time.time())+".jpg",frame)
...
def main():
    ...
    # images.append(cv2.imread('imgs/3_3.jpg'))
    # out = main_main(images)
